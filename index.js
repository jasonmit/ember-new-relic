...
module.exports = {
  ...
  included: function() {
    ...
    var options =
      (this.app && this.app.options && this.app.options['ember-new-relic']) ||
      {};

    var newRelicConfig = (this.newRelicConfig = this.getNewRelicConfig(
      this.project.config(env).newRelic
    ));

    var importToVendor = (this.importToVendor =
      'importToVendor' in options
        ? options.importToVendor
        : this.importToVendor);
    var outputPath = (this.outputPath =
      'outputPath' in options ? options.outputPath : this.outputPath);
    var isValidNewRelicConfig = (this.isValidNewRelicConfig =
      newRelicConfig.applicationID && newRelicConfig.licenseKey);

    // Don't show the warning if they didn't provide an applicationID
    var showLackOfPropertiesWarning =
      newRelicConfig.applicationID && !isValidNewRelicConfig;
    if (showLackOfPropertiesWarning) {
      this._writeWarnLine(
        'New Relic config needs `applicationId` and `licenseKey` properties in order to output New Relic script.'
      );
    }

    if (!importToVendor && !outputPath) {
      throw this.ui.writeError(
        new Error('Cannot load external new-relic script from undefined output')
      );
    }
  ...
  */
  ...
  writeTrackingCodeTree: function(tree) {
    ...
    if (outputPath && isValidNewRelicConfig) {
      file = writeFile(
        outputPath,
        this.getNewRelicTrackingCode(newRelicConfig)
      );
    }
